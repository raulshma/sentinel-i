import { existsSync } from "node:fs";
import { resolve } from "node:path";
import { config } from "dotenv";
import { z } from "zod";

const envPaths = [
  resolve(process.cwd(), ".env"),
  resolve(process.cwd(), "../../.env"),
];

const envPath = envPaths.find((p) => existsSync(p));
config(envPath ? { path: envPath } : undefined);

const envSchema = z.object({
  NODE_ENV: z
    .enum(["development", "test", "production"])
    .default("development"),
  PORT: z.coerce.number().int().positive().default(8080),
  CLIENT_ORIGIN: z.string().url().default("http://localhost:5173"),
  DATABASE_URL: z
    .string()
    .min(1)
    .default("postgresql://postgres:postgres@localhost:5432/sentinel_i"),
  VALKEY_URL: z.string().min(1).default("redis://localhost:6379"),
  VALKEY_COMMAND_TIMEOUT_MS: z.coerce.number().int().positive().default(2_500),
  AI_PROVIDER: z.enum(["openrouter"]).default("openrouter"),
  OPENROUTER_API_KEY: z.string().optional(),
  OPENROUTER_BASE_URL: z.string().url().default("https://openrouter.ai/api/v1"),
  OPENROUTER_MODEL: z
    .string()
    .min(1)
    .default("nvidia/nemotron-3-super-120b-a12b:free"),
  GEOCODE_API_KEY: z.string().optional(),
  GEOCODE_BASE_URL: z.string().url().default("https://geocode.maps.co"),
  CRAWL4AI_API_URL: z.string().optional(),
  CRAWL4AI_API_KEY: z.string().optional(),
  RSS_FEED_URLS: z.string().optional(),
  RSS_SYNC_CRON: z.string().default("*/15 * * * *"),
  RSS_FEED_FETCH_TIMEOUT_MS: z.coerce.number().int().positive().default(12_000),
  ARTICLE_FETCH_HOST_ALLOWLIST: z.string().optional(),
  PROCESSING_EVENT_DRAIN_TIMEOUT_MS: z.coerce
    .number()
    .int()
    .positive()
    .default(5_000),
  HTTP_FETCH_TIMEOUT_MS: z.coerce.number().int().positive().default(10_000),
  CRAWL4AI_TIMEOUT_MS: z.coerce.number().int().positive().optional(),
  DEV_TOOLS: z
    .enum(["true", "false"])
    .default("false")
    .transform((v) => v === "true"),
});

const parsedEnv = envSchema.parse(process.env);

export const isDevToolsEnabled = parsedEnv.DEV_TOOLS;

export const env = {
  ...parsedEnv,
  CRAWL4AI_TIMEOUT_MS: parsedEnv.CRAWL4AI_TIMEOUT_MS ?? 15_000,
};
